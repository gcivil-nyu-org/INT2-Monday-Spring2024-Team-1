...
class HospitalTests(TestCase):
    def setUp(self):
        self.user = User.objects.create_superuser(email='testuser@test.com', password='12345')
    ...
    def test_update_hospital_status(self):
        ...
        response = self.client.post(reverse('update_hospital_status', args=[hospital.id]), {'status': 'inactive'})
        self.assertEqual(response.status_code, 200)
    ...
    def test_update_hospital_status_invalid(self):
        hospital = Hospital.objects.get(name="General Hospital")
        response = self.client.post(reverse('update_hospital_status', args=[hospital.id]), {'status': 'unknown'})
        self.assertEqual(response.status_code, 400)
