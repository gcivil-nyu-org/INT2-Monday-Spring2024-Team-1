...
from healthScore.views import edit_user_info, homepage, login_view, view_health_history, view_report, view_user_info, view_health_history_requests
...
class viewHealthHistoryTestCase(TransactionTestCase):
    ...
    def setUp(self):
        ...
        s = len(list(Hospital.objects.all().values()))
        if(len(list(Hospital.objects.all().values())) == 0):
